...
import { SignUpRequest, signUpRequestSchema } from '../../models/requests/SignUpRequest';
...
const SignUpPage: NextPage = () => {
	...
	const {
		...
		handleSubmit,
		formState: { errors },
	} = useForm<SignUpRequest>({ resolver: joiResolver(signUpRequestSchema) });
	const onSubmit: SubmitHandler<SignUpRequest> = (data) => {
		api
			.post<OkResponse>('auth/signUp', data)
			.then((response) => {
				router.reload();
			})
				...
				<main className='bg-white w-96 p-5 rounded-lg border-gray-200 border'>
					...
					<p className='text-sm text-center'>
						...
						<Link href='/auth/signIn'>
							<a className='text-indigo-500 underline hover:bg-indigo-50'>Sign in instead</a>
						</Link>
					</p>
				</main>
			</div>
...
export default withAuthGuard(false, '/')(SignUpPage);
